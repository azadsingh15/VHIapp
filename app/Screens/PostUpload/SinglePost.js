...
const SinglePost = ({ route, navigation }) => {
  ...
  const [imageDim, setimageDim] = useState({ height: 500, width: 500 });
  ...
  let nameParts = singlePost.SinglePost.image.split(".");
  let fileParts = singlePost.SinglePost.image.split("/");
  ...
  const pickFromCamera = async () => {
    ...
    if (granted) {
      ...
      if (!data.cancelled) {
        ...
        setimageDim({ height: data.height, width: data.width });
      }
    } else {
  ...
  const pickImage = async () => {
    ...
    if (granted) {
      ...
      if (!result.cancelled) {
        ...
        setimageDim({ height: result.height, width: result.width });
      }
    } else {
  ...
  const gotoImage = () => {
    if (fileType === "pdf") {
      // console.log("in");
      try {
        Linking.openURL(`http://192.168.1.5:3000/${fileName}`);
      } catch (error) {
      ...
      navigation.navigate("SingleImage", {
        ImagePicked:
          ImagePicked !== null ? ImagePicked : singlePost.SinglePost.image,
        imageDim: imageDim,
      });
    }
  };
  // {
  //   ImagePicked: singlePost.SinglePost.image,
  //   imageDim: imageDim,
  // }
  return (
    <View style={styles.container}>
      <View style={styles.imagecontainer}>
        <TouchableOpacity onPress={gotoImage} style={styles.uploadBtnContainer}>
          {
            <Image
              source={{
                uri:
      ...
      <View>
        ...
        <View style={styles.action}>
          ...
          <TextInput
            value={display_name}
            placeholder={singlePost.SinglePost.display_name}
        ...
        <View style={styles.action}>
          ...
          <TextInput
            value={desc}
            placeholder={singlePost.SinglePost.desc}
            style={styles.textInput}
            autoCapitalize="none"
